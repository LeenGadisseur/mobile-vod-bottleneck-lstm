...
parser.add_argument("--datasets", type=str, default = DATASET_PATH_VID_MOUNTED , help="The root directory of the VOC dataset or Open Images dataset.")
...
def load_dataset(seq = 1):
	train_transform = TrainAugmentation(config.image_size, config.image_mean, config.image_std)
	target_transform = net.MatchPrior(config.priors, config.center_variance, config.size_variance, 0.5)

	if seq ==1:
		train_dataset = EPFLDataset(args.datasets, args.cache_path, transform=train_transform,
								 target_transform=target_transform)
		val_dataset = EPFLDataset(args.datasets, args.cache_path, transform=test_transform,
